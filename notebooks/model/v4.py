...
def select_num_samples(dataset, n_samples, cls_to_idx):
    ...
    for key,_ in cls_to_idx.items():
        indices = np.where(targets == key)[0]
...
class CIFAR10(torch.utils.data.Dataset):
    def __init__(
        ...
        self.dataset = tv.datasets.CIFAR10(root=self.path, train=train, download=download)
        self.transfrm = transform

        clas_to_index = { c : i for i, c in enumerate(cls)}
        if len(cls)<10:
                self.dataset = select_classes(self.dataset, cls)
        if (train_samples > 0) and (n_samples is not None):
            self.dataset = select_num_samples(self.dataset, n_samples, clas_to_index)

        self.dataset.targets = torch.tensor([clas_to_index[clas] for clas in self.dataset.targets])#.item()

        self.data = self.dataset.data / 255.0
        self.targets = F.one_hot(self.dataset.targets, len(cls)).numpy()
...
data_train = CIFAR10(path_root= "data/",
            train=True, n_samples=train_samples, cls=classes_train
        )


data_test = CIFAR10(path_root = "data/", train=False, cls=classes_train)
